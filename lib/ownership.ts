...
export async function ownedStore(request:Request,storeId:string){
 ...
 const {data,error}=await db().from('stores').select('id,hostname,name,description,owner_id,status,license_expires_at,verified_at,verification_token').eq('id',storeId).eq('owner_id',user.id).maybeSingle();
